...
class PolynomialRegression:
    ...
    def polyfeatures(X: np.ndarray, degree: int) -> np.ndarray:
        ...
        for i in range(1, degree):
            X_expand.append((X**(i+1)).flatten())
        polyfeatures_of_X = np.array(X_expand).T
        return polyfeatures_of_X
...
def learningCurve(
    ...
    for i in range (1, n):
        train_feature = Xtrain[0:(i+1)]
        train_label = Ytrain[0:(i+1)]

        w = model.fit(train_feature, train_label)

        train_prediction = model.predict(train_feature)
        test_prediction = model.predict(Xtest)

        errorTrain[i] = mean_squared_error(train_prediction, train_label)
        errorTest[i] = mean_squared_error(test_prediction, Ytest)
