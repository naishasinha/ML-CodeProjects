...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    list_one = vanilla_matmul(vanilla_transpose(A), x)
    list_two = vanilla_matmul(A,x)
    list_three = vanilla_matmul(vanilla_transpose(B), y)

    temp = [sum(value) for value in zip(list_one, list_two)]
    result = [sum(value) for value in zip(temp, list_three)]
...
def numpy_solution(
    ...
    """
    return (A.transpose() + A) @ x + B.transpose() @ y
